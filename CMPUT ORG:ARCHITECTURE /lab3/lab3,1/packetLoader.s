...
check:
	...
	and	$t2 $t1 $t8	#$t2 <- upper half word
	and	$t1 $t1 $t9	#$t1 <- lower half word
...
start:#done the first addition, and loop the following addition
	addi	$t0 $t0 2	#plus t0 by 2 byte, previous address plus by 2
...
skip:#when meet the half word of Header Checksum, Skip it
	addi	$t7 $t7 1	#plus the counter by 1
	j	start
