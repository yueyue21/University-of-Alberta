...
int main(int argc, char *argv[]){
  long letter, word;
  char term[200][40];
  int right[200],length[200],space_num[200];
  ...
  int x=0,y=0;                                // index of the array that contains the matched step for each,
  int isNewWord;
  FILE *fpr;
  FILE *fpc;
  ...
  if (argc <=5){
    ...
    if(iscf == 1){          
      ...
      while (y != 200){//??????????????????????????/small buffer is ok for some cases 200 is supposed to be a
	space_num[y] = 0;   // create a array that contain all 0 with a+1 positions
	y++;
      }
	...
	if(ck != '\n'){
	  ...
	  length[a]=b-1;//length stores the length of each term by order given 
	  a++;
	  b = 0;
	}
      }
  ...
  //----------------------output4-----------------there are -f and -l-----------
	...
	if(tolower(ch) == term[c][right[c]]){   // compare the current ch with word term[c] at position of right[c], where right[c] is how many letter matched.
	  if(length[c] == right[c]){
	    printf("%ld,%ld,%s\n",letter - 3, word-space_num[c], term[c]);
	    right[c] = 0;   // if print, then clear the number in position [c].
	  }
	  right[c]++;   // the array right in position c(for the word) will store how further they matched
	}
