...
class Table:
    ...
    def load(self):
        ...
        elif self.__type == Table.INVERTED_INDEX:
    ...
    def save(self):
        if self.__type == Tabel.DATA_TYPE:
            file = open("data/" + Table.__file_name, "w")
        elif self.__type == self.CONFIGURATION_TYPE:
            file = open("config/" + Table.__file_name, "w")
        elif self.__type == self.INVERTED_INDEX:
            file = open("inverted/" + Table.__file_name, "w")            
    ...
    def append(self, row):
        ...
        for existing_row in self.__rows:
            if existing_row[key_index] == row[key_index]:
    ...
    def key_search(self, key):
        ...
        for i in range(0, len(self.__rows)):
            if self.__rows[i][key_index] == key:
                self.__current_pointer = i
