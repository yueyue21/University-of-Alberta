...
int main(int ac, char *av[])
{
	...
	/*
	if ( pthread_create(&ship_tr[0], NULL, ship_animate, &ship1[0])){
...
int setup(int nstrings, struct propset props[])
{
	...
	mvprintw(LINES-1,0,"'Q' to quit, Remaining Rackets: Number of Saucers missed: '0'..'%d' to bounce",num_msg-1);
...
{
	struct ship *info = arg;
	while(1)
	{
		usleep(info->delay*TUNIT);
		move(LINES-2,info->pre_col);			
		addch(' ');
		move(LINES-2,info->current_col);
		addch('|');
	}
}
