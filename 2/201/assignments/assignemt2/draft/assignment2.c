...
int main(int argc, char *argv[]){ 
  ...
  FILE *readfp=NULL; 	/*declare the readfile pointer*/
  ...
  int *right,*length;/* memory pointer to store the extent of matched term, the term length and #of spaces in term*/
  long *space_num;/*#of spaces in term*/
  ...
  for(int i=1; i<argc; i++)
	...
	{	
      ...
      else{search_term = argv[i];}	/*just for the case that input is a term,here use termfp point this term.LS==0*/	
    }
...
void search_nonearby(FILE*readfp,char **term,long letter,long word,int *right,int *length,long str1,long *space_num){
  char ch;
  long isNewWord = 1, i = 0;/* term[i]=ith term in the array*/
  while((ch = fgetc(readfp)) != EOF){
    letter++;
    if (isalnum(ch)){
      if(isNewWord){
