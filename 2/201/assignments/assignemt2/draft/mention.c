...
void search_nearby_single_term(FILE*readfp,FILE*readfp1,char*search_term,long letter,long word,char *nb,int *tdis);
void search_nearby_single_char(FILE*readfp,FILE*readfp1,char*search_term,long letter,long word,char *nb,int *cdis);
int main(int argc, char *argv[]){ 
  int FS=0,LS=0,BS=0,CTS=0,TS=0,CS=0;/*set a switch to indicate if there is -f,-l,-b,-t or -c,-t,-c option*/
  FILE *readfp=NULL,*readfp1=NULL; 	/*declare the readfile pointers*/
	...
	{	
	  ...
	  switch(argv[i][1])
		...
		readfp=fopen(&argv[i][2],"r");
                readfp1=fopen(&argv[i][2],"r");	
	      }	
	      else{
		readfp=fopen(argv[i+1],"r");	/*open readfile*/
		readfp1=fopen(argv[i+1],"r");
	      }
  ...
  if(LS==1){   /*we have -l option*/
	...
	*tdis=5;/*regard the five is get from input,using term nearby function to excute*/
	search_nearby_single_term(readfp,readfp1,search_term,letter,word,nb,tdis);//1
      }
      else{
	if(CS==1){
	  search_nearby_single_char(readfp,readfp1,search_term,letter,word,nb,cdis);//2
	}
	else if(TS==1){
	  search_nearby_single_term(readfp,readfp1,search_term,letter,word,nb,tdis);
	}
      }
    }
  }
...
void search_nearby_single_term(FILE*readfp,FILE*readfp1,char*search_term,long letter,long word,char *nb,int *tdis){
  char ch;
  long size=500;
  int isNewWord=1,i=0,index=0;/*a switch, a peremeter for counted number*/
  char *nearterm;/*make a array of pointers points to the location of nearby terms in unit of term*/
  nearterm=malloc(size*sizeof(long));
  memset(nearterm,0,size*sizeof(long));

  while ((ch =fgetc(readfp1)) !=EOF){
    printf("//////2131///\n");
    exit(1);
    letter++;
    if (isalnum(ch)){
...
/*-----------------------------------------------------------------------------------*/
void search_nearby_single_char(FILE*readfp,FILE*readfp1,char*search_term,long letter,long word,char *nb,int *tdis){
