...
void convert(FILE* landscape, FILE* excutable)
{
  ...
  while(fgets(line, 128, landscape) != NULL){
	...
	x[current_line]=currentx;
	y[current_line]=currenty;
  ...
  total_land_points=current_line;
  fprintf(excutable,"setColor 200 130 0\n");
...
void handle_timeout(int signal)
{
  ...
  if (signal == SIGALRM)
	...
	fprintf(excutable,"eraseSegment %ld %ld %ld %ld\n",lround(cx+dx-midx),lround(cy+dy-midy),lround(dx),lround(dy));*/
      ...
      thrust1=0;
	//horizontal beyond incapability
      if(ax>=630 ||bx>=630||cx>=630||dx>=630)
	vx=0;
     // move(10,10);
     // printw("%d %d,%d %d,%d %d,%d %d,%d %d,%d %d,",x[0],y[0],x[1],y[1],x[2],y[2],x[3],y[3],x[4],y[4],x[5],y[5]);
      for(int k=0;k<total_land_points;k++){
		if(linecross(ax,ay,bx,by,x[k],y[k],x[k+1],y[k+1])){
			stop=1;
			break;	
		}	
		else if (linecross(bx,by,cx,cy,x[k],y[k],x[k+1],y[k+1])){
			stop = 1;
			break;		
		}
		else if (linecross(cx,cy,dx,dy,x[k],y[k],x[k+1],y[k+1])){
			stop = 1;
			break;		
		}
		else if (linecross(dx,dy,ax,ay,x[k],y[k],x[k+1],y[k+1])){
			stop = 1;
			break;		
		}
	}
