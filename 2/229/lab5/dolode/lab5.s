...
#---------------------------------------------------------------------------------------------------------------------------------------					
		first2_done:
				la	$t8 temp
				lw	$t9 8($t8)				#t9 have the size of array1
				la	$t8 array3
				li	$t7 0 
				li	$t4 0 
				
			loop3:
					beq	$t7 0 first_setup
					beq	$t7 $t9 set_up_done	#the rest already set to 000000000000000000000000000
			...
			cmput_current_set:
					...
					#j	set_current    ----------------------------this line modified
			first_setup:
					li	$t0 1
					sw	$t0 0($t8)	
					j	fist_set_back
			shift_set:
					beq	$t4 $t1 have_left_number
					sll	$t0 $t0 1
					addi	$t4 $t4 1
					j	shift_set
			have_left_number:
