...
def allarminage():
    ...
    #aa -- Find the abnormal rate for each test type
    aa="SELECT   t1.type_id, count(distinct t1.patient_no)/count(distinct t2.patient_no) ab_rate "
    bb="FROM     test_record t1, test_record t2 "
    cc="WHERE    t1.result <> 'normal' AND t1.type_id = t2.type_id "
    dd="GROUP BY t1.type_id"
    g=aa+bb+cc+dd
    curs.execute(g)
    row = curs.fetchall()
    while row:
    ...
    #bbb --- Find the patient count above each age
    bbb="SELECT  t1.type_id,age,COUNT(distinct p1.health_care_no) AS cnt FROM    patient p1, test_record t1, (SELECT DISTINCT trunc(months_between(sysdate,p1.birth_day)/12) AS age FROM patient p1) WHERE trunc(months_between(sysdate,p1.birth_day)/12)>=age AND p1.health_care_no=t1.patient_no GROUP BY age,t1.type_id"    
